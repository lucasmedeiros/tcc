...
dados_consumo_por_horario_mes = dados_liteme_pow_com_data_filtrados %>%
  ...
  summarise(
    ...
    desvio = sd_trim((potencia_total) / 1000, trim = 0.25),
    .groups = "drop"
  )
...
dados_consumo_por_horario_mes %>%
  ...
  scale_x_discrete(limits = c(seq(0, 23))) +
  facet_wrap(~mes, labeller = as_labeller(meses),ncol = 2) + 
  labs(title = "Consumo médio (truncado 25%) por horário do dia agrupado por mês",
       x = "Horário do dia",
       y = "Consumo médio (KW/h)")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10}
consumo_mais_temperatura_exp %>%
  filter(dia_sem_ano < '02-14') %>% 
  ggplot(aes(x = consumo, y = temperatura)) +
  geom_point(color = "gray48") +
  geom_smooth(method = lm, se = FALSE, color = "black") +
  labs(x = "Consumo do condicionador de ar (kWh)",
       y = "Temperatura Fevereiro (ºC)",
       title = "Gráfico de temperatura de CG x consumo do condicionador de ar período do experimento")
```
...
sumario_consumo_exp %>% 
  ...
  ylim(1, 3) +
  labs(title = "Consumo médio do PowLatec por dia (2ª semana)",
       x = "Dia da análise",
       y = "Consumo médio (KW/h)")
```
...
sumario_consumo_exp %>%
  ...
  ylim(0, 50) +
  labs(title = "Consumo total do PowLatec por dia (2ª semana)",
       y = "Consumo total (KWh)",
       x = "Dia da análise")
```
...
dados_th_latec_exp_com_data %>%
  ...
  labs(x = "Dia da análise",
       y = "Temperatura da sala (ºC)",
       title = "Gráfico da média da temperatura da sala por dia (2ª semana)")
```
...
sumario_th_latec_exp %>%
  ...
  labs(x = "Horário do dia (h)",
       y = "Temperatura da sala (ºC)",
       title = "Gráfico da média da temperatura da sala durante os dias da semana (2ª semana)") +
  facet_wrap(~dia, ncol = 2)
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10}
consumo_mais_temperatura_exp %>%
  filter(dia_sem_ano >= '02-14' & dia_sem_ano < '02-20') %>% 
  ggplot(aes(x = consumo, y = temperatura)) +
  geom_point(color = "gray48") +
  geom_smooth(method = lm, se = FALSE, color = "black") +
  labs(x = "Consumo do condicionador de ar (kWh)",
       y = "Temperatura Fevereiro (ºC)",
       title = "Temperatura de CG x consumo do condicionador de ar (2ª semana de experimento)")
```

```{r, echo=FALSE, warning=FALSE}
consumo_mais_temperatura_exp %>%
  filter(dia_sem_ano >= '02-13' & dia_sem_ano < '02-20') %>% 
  group_by() %>% 
  summarise(corr_pearson = cor(consumo, temperatura, method = 'pearson'))
```
